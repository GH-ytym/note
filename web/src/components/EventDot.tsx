...
export default function EventDot({ item, selected = false, onSelect }: { item: CalendarItem; selected?: boolean; onSelect?: (item: CalendarItem) => void }) {
	const done = isEventDone(item);
	const calendarEvent = item.kind === "event";

  return (
    <button
      data-search-key={`${item.kind}-${item.eventId || item.todoId}`}
