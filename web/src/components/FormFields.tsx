...
export function CustomColorSwatch({ value, selected, onChange }: { value: string; selected: boolean; onChange: (value: string) => void }) {
  return (
...
export function TimeField({ label = "时间", value, onChange, compact = false }: { label?: string; value: string; onChange: (value: string) => void; compact?: boolean }) {
  const [open, setOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    function handleNativeWheel(event: WheelEvent) {
      ...
      const unitElement = event.target instanceof Element
        ? event.target.closest<HTMLElement>("[data-time-unit]")
        : null;
    ...
    return () => {
      ...
      popover.removeEventListener("wheel", handleNativeWheel);
    };
  }, [open]);

  function adjust(unit: string, amount: number) {
    setDraft((current) => {
      const { hour, minute } = splitTime(current);
      return unit === "hour"
        ? joinTime(hour + amount, minute)
...
export function ColorField({ value, onChange, allowRandom = false }: { value: string; onChange: (value: string) => void; allowRandom?: boolean }) {
  const [open, setOpen] = useState(false);
