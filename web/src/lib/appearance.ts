...
export interface Appearance { backgroundColor: string; themeColor: string; opacity: number }
...
export function normalizeAppearance(value: Record<string, unknown> = {}): Appearance {
...
export function normalizeSettings(value: Record<string, unknown> = {}): Settings {
  const appearance = normalizeAppearance(value);
  const clockTracks = Number(value.clockTracks);
  return {
    ...appearance,
    miniViewMode: oneOf(value.miniViewMode, ["clock", "timeline", "tags"], DEFAULT_PREFERENCES.miniViewMode),
    searchSplit: typeof value.searchSplit === "boolean" ? value.searchSplit : DEFAULT_PREFERENCES.searchSplit,
    searchLimit: Number.isFinite(Number(value.searchLimit)) ? Math.max(1, Math.min(100, Math.round(Number(value.searchLimit)))) : DEFAULT_PREFERENCES.searchLimit,
...
function readable(color: string, surfaces: string[], ink: string) {
...
export function appearanceTokens(settings: Appearance) {
