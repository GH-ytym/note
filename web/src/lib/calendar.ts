import type { RepeatMode, NotifyMode, TodoOccurrence, EventOccurrence, CalendarResponse, CalendarItem, Todo } from "../types";
export const WEEKDAYS = ["一", "二", "三", "四", "五", "六", "日"];
export const SHANGHAI_TIME_ZONE = "Asia/Shanghai";

export const REPEAT_VALUES: Record<string, RepeatMode> = {
  仅一次: "once",
  每天: "daily",
  工作日: "weekdays",
  周末: "weekends",
...
export const REMINDER_VALUES: Record<string, NotifyMode> = {
  静默提醒: "silent",
  弹窗提醒: "popup",
};
...
export function dateKeyFromUTC(date: Date) {
...
export function preciseDateLabel(dateKey: string) {
  const date = dateFromKey(dateKey);
  return `${date.getUTCFullYear()}年${date.getUTCMonth() + 1}月${date.getUTCDate()}日`;
}

export function calendarNote(dateKey: string) {
...
export function buildGridOutlinePath(indices: number[]) {
...
export function joinTime(hour: number, minute: number) {
  return `${pad((hour + 24) % 24)}:${pad((minute + 60) % 60)}`;
}

export function shortMonthLabel(month: Date) {
...
export function buildPeekDays(month: Date, edge: string) {
  if (edge === "end") {
...
export function dateTimeAt(dateKey: string, time = "00:00") {
  return new Date(`${dateKey}T${time}:00+08:00`).toISOString();
}

export function shanghaiDateTimeParts(value: string | Date) {
  ...
  const part = (type: string) => parts.find((itemPart) => itemPart.type === type)?.value;
...
export function calendarTodoFromOccurrence(item: TodoOccurrence): CalendarItem {
...
export function calendarEventFromOccurrence(item: EventOccurrence): CalendarItem {
...
export function calendarItemsFromResponse(result: CalendarResponse): CalendarItem[] {
...
export function readableSelectionInk(color: string) {
  const hex = (color || "#F3B51B").replace("#", "");
  const value = Number.parseInt(hex, 16);
...
export function todoDateKeys(todo: Todo) {
