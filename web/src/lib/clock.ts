...
export function clockPoint(minutes: number, radius: number) {
  const angle = minutes / 1440 * Math.PI * 2 - Math.PI / 2;
  return { x: 180 + Math.cos(angle) * radius, y: 180 + Math.sin(angle) * radius };
}
export function clockMinute(x: number, y: number) {
  return ((Math.atan2(y - 180, x - 180) + Math.PI / 2 + Math.PI * 2) % (Math.PI * 2)) / (Math.PI * 2) * 1440;
}
export function minuteDelta(previous: number, next: number) {
  let delta = next - previous;
  if (delta > 720) delta -= 1440;
  if (delta < -720) delta += 1440;
  return delta;
}
export function clockArc(start: number, end: number, radius: number) {
...
export function clockTrackRadius(track: number) {
  return 116 - track * 5.5;
}

export function layoutTodoTracks(items: CalendarItem[], date: string, trackCount = 7) {
  const count = Math.max(1, Math.min(10, Number(trackCount) || 7));
  const tracks: number[][] = Array.from({ length: count }, () => []);
