  ...
  function setNativeDate(value: string) {
    ...
    try {
			const created = kind === "event"
				? await createEvent({ ...common, ends_at: dateTimeAt(form.endDate, form.endTime) })
				: await createTodo({ ...common, notify_mode: REMINDER_VALUES[form.reminder] });
      ...
      setError(createError instanceof Error ? createError.message : "操作失败");
    } finally {
      setSaving(false);
    }
  }
